...
export default {
   ...
   methods: {
      ...
      async reset(event) {
         ...
         try {
            const res = await fetch('http://135.148.24.68:3000/reset', {
               method: 'POST',
               headers: {
   ...
   <div class="w-screen h-screen flex justify-center">
      ...
      <div v-if="successVisible" class="bg-white rounded-lg shadow-lg w-full h-fit my-auto max-w-md absolute top-[15%] -translate-y-1/2">
         ...
         <div class=" flex flex-col p-6 pt-2">
            ...
            <p class="font-bold">We've sent a mail to your email.</p>
            <a href="/login" class="w-fit ms-auto text-green-600 hover:underline">
               Back to login
            </a>
         </div>
      </div> 
      ...
      <div class="bg-white p-8 rounded-lg shadow-lg w-full h-fit my-auto max-w-md">
         <h1 class="text-3xl font-semibold text-left mb-6">Reset password</h1>
         ...
         <p class="text-center text-gray-600 mt-6"><a href="/login" class="text-purple-600 hover:underline">Back to login</a></p>
